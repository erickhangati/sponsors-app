...
export function SponsorRecentPayments() {
    ...
    const fetchRecentPayments = async () => {
      ...
      try {
        ...
        const paymentsResponse = await axios.get(
          `${baseUrl}/sponsors/payments?page=1&page_size=5`,
          {
            headers: {
      ...
      {payments.map((payment) => (
        <div key={payment.id} className="flex items-center">
          <Avatar className="h-9 w-9">
            <AvatarImage src={`/placeholder.svg?height=36&width=36`} alt={payment.child_name} />
            <AvatarFallback>{getInitials(payment.child_name || '')}</AvatarFallback>
          </Avatar>
          <div className="ml-4 space-y-1">
          ...
          <div className="ml-auto flex flex-col items-end gap-1">
            <p className="text-sm font-medium">
              {payment.currency === 'KES' || payment.currency === 'KSH' ? 'KSh' : payment.currency}{' '}
