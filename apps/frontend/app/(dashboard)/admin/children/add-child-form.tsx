...
import { childFormSchema, type ChildFormValues, Gender } from './schema';
...
export function AddChildForm({ onSuccess, onCancel }: AddChildFormProps) {
  ...
  const onSubmit = async (data: ChildFormValues) => {
    ...
    const loadingToast = toast.loading('Creating child account...');
    ...
    try {
      ...
      const response = await axios.post(`${baseUrl}/users`, childData, {
        headers: {
      ...
      toast.success('Child created successfully', {
        description: `${data.first_name} ${data.last_name} has been added as a child.`,
      });
