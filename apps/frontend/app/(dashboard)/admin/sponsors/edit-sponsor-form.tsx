...
import { useForm } from 'react-hook-form'; // Fixed import from react-hook-form instead of react
...
import { sponsorEditSchema, type SponsorEditValues, Gender } from './schema';
...
export function EditSponsorForm({ sponsor, onSuccess, onCancel }: EditSponsorFormProps) {
  ...
  const [dobViewDate, setDobViewDate] = useState<Date>(
    sponsor.date_of_birth ? new Date(sponsor.date_of_birth) : new Date(),
  );

  // Initialize form with sponsor values
  const form = useForm<SponsorEditValues>({
    resolver: zodResolver(sponsorEditSchema),
    defaultValues: {
      ...
      date_of_birth: sponsor.date_of_birth ? new Date(sponsor.date_of_birth) : undefined,
      gender: sponsor.gender as Gender | undefined,
      background_info: sponsor.background_info || '',
      is_active: sponsor.is_active,
    },
  });

  const onSubmit = async (data: SponsorEditValues) => {
    ...
    const loadingToast = toast.loading('Updating sponsor account...');
    ...
    try {
      ...
      // Make the API request - CHANGED FROM patch TO put
      const baseUrl = process.env.NEXT_PUBLIC_BACKEND_API_BASE_URL || 'http://127.0.0.1:8000';
      await axios.put(`${baseUrl}/users/${sponsor.id}`, sponsorData, {
        headers: {
